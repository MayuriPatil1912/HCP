...
export function HcpDataRow({ row, rowKey }: HcpDataRowProps) {
  return (
    <div className="table-row hcp-row" data-row-key={rowKey}>
      {/* ID */}
      ...
      {/* Specialty */}
      <div className="table-cell">{row.specialty ?? "—"}</div>

      {/* Calls */}
      <EditableCell rowKey={rowKey} value={row.calls} />

      {/* TRx */}
      <div className="table-cell numeric">{row.trx.toLocaleString()}</div>

      {/* NRx */}
      <div className="table-cell numeric">{row.nrx.toLocaleString()}</div>

      {/* CPI */}
      <div className="table-cell numeric">{formatCpi(row.calls, row.trx)}</div>
    </div>
  );
}
